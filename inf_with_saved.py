...
from seq2seq.helpers import sequence_accuracy, cal_recall, strip_pad
...
from seq2seq.predict import (predict_and_save, predict_and_save_beamsearch, predict_and_save_noeos, predict_and_save_transformer, 
                                predict_and_save_noeos_transformer, predict_and_save_lenreg, predict_and_save_eos_distribution, predict_beamsearch, predict_beamsearch_disc, predict_beamsearch_large, predict_disc, predict_sampling)
...
def main(flags):
    ...
    if flags["mode"] == "train":
        ...
        for split in splits:
            ...
            # for iter_time in tqdm(range(1,11)):
            for iter_time in tqdm(range(1,836)):
                # count += 1
                # if count % 10 == 0:
                ...
                #                                     times = times,
                #                                     )
                with open("./saved_res/decode_res_times%d_iter%d"%(times, iter_time), 'rb') as f:
                    result_list1 = pickle.load(f)
                with open("./saved_res/greedy_decode_res_times%d_iter%d"%(times, iter_time), 'rb') as f:
                ...
                for i in range(len(result_list1)):
                    result1 = result_list1[i]
                    result2 = result_list2[i]

                    input_sequence1, derivation_spec1, situation_spec1, output_sequence1, target_sequence1, situation1 = result1
                    input_sequence2, derivation_spec2, situation_spec2, output_sequence2, target_sequence2, situation2 = result2

                    situation = situation1
                    input_sequence = input_sequence1
                    target_sequence = target_sequence1
                    output_sequence = output_sequence1 + output_sequence2
