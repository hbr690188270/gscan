...
from seq2seq.my_model import PredNet, PredNetTransformerV2, PredNetTransformer
from seq2seq.predict import predict_and_save, predict_and_save_noeos, predict_and_save_transformer, predict_and_save_noeos_transformer, predict_length, predict_withlenbias_and_save
from seq2seq.evaluate import evaluate_length_pred
...
parser.add_argument("--model_type", type=str, default='orig', choices=['orig', 'transformer', 'transformerv2'],)
parser.add_argument("--inf_type", type=str, default='orig', choices=['orig', 'no_eos'],)
...
def main(flags):
    ...
    for split in splits:
        ...
        l2_distance, l1_distance, l2_list, l1_list, target_len_list = evaluate_length_pred(
                test_set.get_data_iterator(batch_size=300), model=model,
                max_decoding_steps=1, pad_idx=test_set.target_vocabulary.pad_idx,
                sos_idx=test_set.target_vocabulary.sos_idx,
                eos_idx=test_set.target_vocabulary.eos_idx,
                max_examples_to_evaluate=flags["max_testing_examples"])        
        logger.info("  L2 distance: %5.2f L1 distance: %5.2f " % (l2_distance, l1_distance))

        for i in range(len(l1_list)):
            target_len = target_len_list[i]
            if target_len not in l1_distance_dict:
                l1_distance_dict[target_len] = [l1_list[i]]
                l2_distance_dict[target_len] = [l2_list[i]]
            else:
                l1_distance_dict[target_len].append(l1_list[i])
                l2_distance_dict[target_len].append(l2_list[i])
    with open("saved_results/distance_transformer.pkl",'wb') as f:
        pickle.dump((l1_distance_dict, l2_distance_dict), f)
