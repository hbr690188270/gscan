...
from seq2seq.gSCAN_dataset import GroundedScanDataset, GroundedScanDatasetForTest
...
parser.add_argument("--model_type", type=str, default='orig', choices=['cpg', 'orig', 'transformer'],)
    ...
    if flags["mode"] == "train":
        ...
        for split in splits:
            ...
            if flags['model_type'] == 'transformer':
                if flags['inf_type'] == 'no_eos':
                    output_file = predict_and_save_noeos_transformer(dataset=test_set, model=model, output_file_path=output_file_path, **flags)
                else:
                    output_file = predict_and_save_transformer(dataset=test_set, model=model, output_file_path=output_file_path, **flags)
            else:
                if flags['inf_type'] == 'no_eos' or flags['no_eos']:
                    output_file = predict_and_save_noeos(dataset=test_set, model=model, output_file_path=output_file_path, **flags)
                elif flags['inf_type'] == 'len_reg':
                    output_file = predict_and_save_lenreg(dataset=test_set, model=model, output_file_path=output_file_path, **flags)
                elif flags['inf_type'] == 'eos_distribution':
                    output_file = predict_and_save_eos_distribution(dataset=test_set, model=model, output_file_path=output_file_path, **flags)
                elif flags['inf_type'] == 'beam_search':
                    output_file = predict_and_save_beamsearch(dataset=test_set, model=model, output_file_path=output_file_path, **flags)
                else:
