...
class MyTransformerDecoderLayer(TransformerDecoderLayer):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
                image_hidden: Tensor = None, 
                tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ## multihead attn:  input shape:   seq_len, batch_size, hidden_dim
        ...
        encoder_tgt = self.multihead_attn(tgt, memory, memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        image_tgt = self.multihead_attn2(tgt, image_hidden, image_hidden,)[0]

        tgt = tgt + self.dropout2(encoder_tgt) + self.dropout2(image_tgt)
        tgt = self.norm2(tgt)
...
class MyTransformerDecoderLayer_v2(TransformerDecoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        super(TransformerDecoderLayer, self).__init__()
        self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
        ...
        self.norm1 = LayerNorm(d_model)
        self.norm2 = LayerNorm(d_model)
        self.norm2_1 = LayerNorm(d_model)
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
        ...
        tgt = tgt + self.dropout2_1(encoder_tgt)
        tgt = self.norm2_1(tgt)

        image_tgt = self.multihead_attn2(tgt, image_hidden, image_hidden,)[0]
        tgt = tgt + self.dropout2(image_tgt)
        tgt = self.norm2(tgt)
...
class DecoderTransformer(nn.Module):
    def __init__(self, hidden_size: int, output_size: int, num_layers: int,
                dropout_probability=0.1, padding_idx=0, image_hidden_dim = 150,
                ):
        ...
        self.image_hidden_transform = nn.Linear(image_hidden_dim, hidden_size)

        self.embedding_norm = LayerNorm(hidden_size)
        self.padding_idx = 0
        self.transformer_decoder = MyTransformerDecoder(decoder_layer = MyTransformerDecoderLayer(d_model = hidden_size, nhead = 2, dim_feedforward = 4 * hidden_size, dropout = 0.1),
                                                          num_layers = 1,
                                                        )
        # self.transformer_decoder = MyTransformerDecoder(decoder_layer = MyTransformerDecoderLayer_v2(d_model = hidden_size, nhead = 2, dim_feedforward = 4 * hidden_size, dropout = 0.1),
        #                                                   num_layers = 1,
        #                                                 )

        self.positional_embedding = SinusoidalPositionalEmbedding(embedding_dim = hidden_size, padding_idx = self.padding_idx, init_size = 100)
        
        self.hidden_to_output = nn.Linear(hidden_size, output_size)

    def forward_step(self, input_tokens: torch.LongTensor, last_hidden: Tuple[torch.Tensor, torch.Tensor],
                     encoded_commands: torch.Tensor, commands_lengths: torch.Tensor,
                     encoded_situations: torch.Tensor, encoder_mask) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor],
                                                                torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Run batch decoder forward for a single time step.
         Each decoder step considers all of the encoder_outputs through attention.
        ...
        # embedded_input = embedded_input.unsqueeze(0)  # [1, batch_size, hidden_size]
        embedding = self.embedding(input_tokens)  # [batch_size, hidden_size]
        pos_embed = self.positional_embedding(input_tokens)
        embedding += pos_embed
        embedding = self.embedding_norm(embedding)
        embedding = self.dropout(embedding)
        embedding = embedding.permute([1, 0, 2])  ## seq_len, batch_size, hidden_size

        tgt_key_padding_mask = input_tokens.eq(self.padding_idx)
        future_mask = self.buffer_future_mask(embedding)

        encoded_situations = self.image_hidden_transform(encoded_situations)

        output_hidden = self.transformer_decoder(tgt = embedding, memory = encoded_commands, 
                                                tgt_key_padding_mask = tgt_key_padding_mask,
                                                memory_key_padding_mask = encoder_mask, 
                                                image_hidden = encoded_situations,
                                                tgt_mask = future_mask,
                                                )
        output = self.hidden_to_output(output_hidden)
    ...
    def forward(self, input_tokens: torch.LongTensor, input_lengths: List[int],
                init_hidden: Tuple[torch.Tensor, torch.Tensor], encoded_commands: torch.Tensor,
                commands_lengths: List[int], encoded_situations: torch.Tensor, encoder_mask) -> Tuple[torch.Tensor, List[int],
                                                                                        torch.Tensor]:
        """
        Run batch attention decoder forward for a series of steps
         Each decoder step considers all of the encoder_outputs through attention.
        ...
        #     (input_tokens.new_zeros(input_tokens.size(0), encoded_commands.size(0)), future_mask), dim = 1
        # )


        encoded_situations = self.image_hidden_transform(encoded_situations)
        encoded_situations = encoded_situations.permute([1,0,2])
        output_hidden = self.transformer_decoder(tgt = embedding, memory = encoded_commands, 
                                                tgt_key_padding_mask = tgt_key_padding_mask,
                                                memory_key_padding_mask = encoder_mask, 
                                                image_hidden = encoded_situations,
    ...
    def buffer_future_mask(self, tensor):
        dim = tensor.size(0)
